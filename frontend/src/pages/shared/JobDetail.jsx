...
export default function JobDetail() {
  ...
  const { messages, setMessages, sendMessage, sendFileMessage, connected } = useSocket(isInChat ? id : null);
  ...
  useEffect(() => {
    if (isInChat) {
      getMessages(id).then(res => setMessages(res.data.messages.map(m => ({
        ...
        text: m.text, file: m.file, timestamp: m.timestamp,
      })))).catch(() => {});
    }
  }, [isInChat, id]);
  ...
  const handleRejectBid = async (bidId, freelancerName) => {
    if (!window.confirm(`Reject bid from ${freelancerName}?`)) return;
    setActionLoading(true); setActionError("");
    try { await rejectBid(bidId); fetchJob(); }
    catch (err) { setActionError(err.response?.data?.message || "Failed"); }
    finally { setActionLoading(false); }
  };

  const handleCancelContract = async (e) => {
    e.preventDefault(); if (!cancelReason.trim()) return;
    setCancelLoading(true);
    try {
      await cancelContract(id, cancelReason);
      setShowCancelModal(false); setCancelReason(""); fetchJob();
  ...
  const handleWithdraw = async () => {
    ...
    try { await withdrawJob(id); setShowWithdrawModal(false); fetchJob(); alert("Job withdrawn."); }
    catch (err) { setActionError(err.response?.data?.message || "Failed"); }
    finally { setWithdrawLoading(false); }
  };

  const handleRaiseDispute = async (e) => {
    e.preventDefault(); if (!disputeReason.trim()) return;
    setDisputeLoading(true);
    try {
      await raiseDispute(id, { reason: disputeReason });
      setShowDisputeModal(false); setDisputeReason(""); fetchJob();
  ...
  const handlePaymentSuccess = () => { setShowPaymentModal(false); fetchJob(); alert("Payment secured!"); };
  ...
  const handleSubmitBid = async (e) => {
    ...
    try {
      ...
      setShowBidModal(false); setBidForm({ amount: "", proposal: "" }); alert("Bid submitted!");
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      ...
      setShowReviewModal(false); setReviewSubmitted(true); alert("Review submitted!");
  ...
  const handleSendFile = async (e) => {
    ...
    try {
      const res = await uploadFile(id, file, setFileProgress);
      sendFileMessage(res.data.file);
    } catch (err) { setFileError(err.response?.data?.message || "Upload failed"); }
    finally { setFileUploading(false); setFileProgress(0); e.target.value = ""; }
  };
    ...
    <div className="job-detail">
      <div className="page-header">
        <div>
          ...
          <div className="job-meta">
            ...
            {hasOpenDispute && <span className="status-badge" style={{ background: "rgba(245,158,11,0.15)", color: "#f59e0b" }}>⚠️ Dispute Open</span>}
        ...
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
          ...
          {isAssignedFreelancer && job.status === "in-progress" && <button className="btn btn--success" onClick={handleMarkComplete} disabled={actionLoading}>Mark Complete</button>}
          {isClient && job.status === "in-progress" && <button className="btn btn--danger" onClick={() => setShowCancelModal(true)}>Cancel Contract</button>}
          {isClient && job.status === "open" && <button className="btn btn--ghost" onClick={() => navigate(`/jobs/${job._id}/edit`)}>✏️ Edit Job</button>}
          {canWithdraw && <button className="btn btn--danger" onClick={() => setShowWithdrawModal(true)} disabled={actionLoading}>Withdraw Job</button>}
          {canDispute && <button className="btn btn--danger" onClick={() => setShowDisputeModal(true)}>⚠️ Raise Dispute</button>}
      ...
      {dispute && (
        <div style={{ padding: "1rem 1.25rem", borderRadius: "8px", marginBottom: "1rem", background: dispute.status === "open" ? "rgba(245,158,11,0.08)" : "rgba(16,185,129,0.08)", border: `1px solid ${dispute.status === "open" ? "rgba(245,158,11,0.3)" : "rgba(16,185,129,0.3)"}` }}>
          <div style={{ fontWeight: 600, marginBottom: "0.25rem" }}>{dispute.status === "open" ? "⚠️ Dispute Under Review" : "✅ Dispute Resolved"}</div>
          <div style={{ fontSize: "0.875rem", color: "var(--text-muted)" }}>Raised by {dispute.raisedBy?.name} · {dispute.reason}</div>
          {dispute.resolution && <div style={{ fontSize: "0.875rem", marginTop: "0.35rem", color: "#10b981" }}>Resolution: {dispute.resolution.replace(/_/g, " ")}{dispute.adminNote && ` — "${dispute.adminNote}"`}</div>}
        </div>
      )}
                ...
                <div key={bid._id} className={"bid-card bid-card--" + bid.status}>
                  ...
                  {bid.status === "pending" && job.status === "open" && (
                    <div style={{ display: "flex", gap: "0.5rem" }}>
                      <button className="btn btn--primary btn--sm" onClick={() => handleAcceptBid(bid)} disabled={actionLoading}>✓ Accept & Pay</button>
                      <button className="btn btn--danger btn--sm" onClick={() => handleRejectBid(bid._id, bid.freelancerId?.name)} disabled={actionLoading}>✕ Reject</button>
                    </div>
                  )}
                </div>
              ))}
      ...
      {isInChat && (
        <div className="content-card chat-card">
              ...
              return (
                <div key={msg._id} className={"message" + (isMine ? " message--mine" : "")}>
                  ...
                  <div className="message-bubble">
                    {msg.file ? <FileMessage file={msg.file} /> : msg.text}
                  </div>
          ...
          {fileError && <div style={{ padding: "0.5rem 1rem", color: "#ef4444", fontSize: "0.8rem" }}>{fileError}</div>}
          {fileUploading && (
            <div style={{ padding: "0.5rem 1rem" }}>
              <div style={{ height: "4px", background: "var(--border)", borderRadius: "2px" }}>
                <div style={{ height: "100%", width: `${fileProgress}%`, background: "var(--accent)", borderRadius: "2px", transition: "width 0.2s" }} />
              </div>
              <div style={{ fontSize: "0.75rem", color: "var(--text-muted)", marginTop: "0.25rem" }}>Uploading... {fileProgress}%</div>
            </div>
          )}
          <form onSubmit={handleSendMessage} className="chat-input-form">
            <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} placeholder="Type a message..." disabled={!connected || fileUploading} />
            <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/gif,application/pdf,.docx" style={{ display: "none" }} onChange={handleSendFile} />
            <button type="button" className="btn btn--ghost btn--sm" onClick={() => fileInputRef.current?.click()} disabled={!connected || fileUploading} title="Attach file" style={{ padding: "0.5rem 0.75rem" }}>📎</button>
            <button type="submit" disabled={!connected || !chatInput.trim() || fileUploading} className="btn btn--primary">Send</button>
          </form>
        </div>
      )}
        ...
        <div className="modal-overlay" onClick={() => setShowBidModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmitBid} className="modal-form">
              <div className="form-group"><label>Bid Amount (USD)</label><input type="number" min="1" value={bidForm.amount} onChange={e => setBidForm({...bidForm, amount: e.target.value})} placeholder={`Budget: $${job.budgetMin} - $${job.budgetMax}`} required /></div>
              <div className="form-group"><label>Proposal</label><textarea value={bidForm.proposal} onChange={e => setBidForm({...bidForm, proposal: e.target.value})} placeholder="Describe your approach..." rows={5} required minLength={20} /></div>
      ...
      {showPaymentModal && clientSecret && (
        <div className="modal-overlay">
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><h2>💳 Secure Payment</h2><button className="modal-close" onClick={() => setShowPaymentModal(false)}>✕</button></div>
      ...
      {showDisputeModal && (
        <div className="modal-overlay" onClick={() => setShowDisputeModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><h2>⚠️ Raise a Dispute</h2><button className="modal-close" onClick={() => setShowDisputeModal(false)}>✕</button></div>
            <form onSubmit={handleRaiseDispute} className="modal-form">
              <div className="cancel-warning"><p>An admin will review your dispute and take appropriate action.</p><p>Please provide as much detail as possible.</p></div>
              <div className="form-group"><label>Reason for dispute *</label><textarea value={disputeReason} onChange={e => setDisputeReason(e.target.value)} placeholder="Describe the issue in detail..." rows={5} required minLength={20} /></div>
              <div className="modal-actions">
                <button type="button" className="btn btn--ghost" onClick={() => setShowDisputeModal(false)}>Cancel</button>
                <button type="submit" className="btn btn--danger" disabled={disputeLoading || disputeReason.trim().length < 20}>{disputeLoading ? "Submitting..." : "Submit Dispute"}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showWithdrawModal && (
        <div className="modal-overlay" onClick={() => setShowWithdrawModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><h2>⚠️ Withdraw Job</h2><button className="modal-close" onClick={() => setShowWithdrawModal(false)}>✕</button></div>
            <div className="modal-form"><div className="cancel-warning"><p>⚠️ This will permanently withdraw this job posting.</p><p>All pending bids will be rejected and freelancers will be notified.</p></div></div>
            <div className="modal-actions">
              <button className="btn btn--ghost" onClick={() => setShowWithdrawModal(false)}>Keep Job</button>
              <button className="btn btn--danger" onClick={handleWithdraw} disabled={withdrawLoading}>{withdrawLoading ? "Withdrawing..." : "Confirm Withdraw"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showCancelModal && (
        <div className="modal-overlay" onClick={() => setShowCancelModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><h2>⚠️ Cancel Contract</h2><button className="modal-close" onClick={() => setShowCancelModal(false)}>✕</button></div>
            <form onSubmit={handleCancelContract} className="modal-form">
              ...
              <div className="form-group"><label>Reason for cancellation *</label><textarea value={cancelReason} onChange={e => setCancelReason(e.target.value)} placeholder="e.g. The freelancer is not responsive..." rows={4} required minLength={10} /></div>
              <div className="modal-actions">
                <button type="button" className="btn btn--ghost" onClick={() => setShowCancelModal(false)}>Keep Contract</button>
                <button type="submit" className="btn btn--danger" disabled={cancelLoading || !cancelReason.trim()}>{cancelLoading ? "Cancelling..." : "Cancel Contract"}</button>
              </div>
            </form>
          </div>
        </div>
